...
! for crop
 ...
 ! for rubber phenology and carbon allocation (Ashehad Ali)
  integer , allocatable :: tapping(:)        !flag for tapping rubber (added by Ashehad Ali)
  integer , allocatable :: clearcut_yr(:)    !year when forest was clearcut
  real(r8), allocatable :: crit_dry_dur(:)   !number of days till rainfall is below rain_min for rubber
  real(r8), allocatable :: rain_min(:)       !rainfall below which leaf growth is switched to leaf litterfall for rubber
  real(r8), allocatable :: tap_npp(:)        !tapping proportional to npp for rubber
  real(r8), allocatable :: tap_partition(:)  !tapping partitioned between growth and storge for rubber
...
!-----------------------------------------------------------------------
    ...
    if ( .not. readv ) call endrun( trim(subname)//' ERROR: error in reading in pft data' )
    !for rubber phenology and allocation (Ashehad Ali)
    call ncd_io('tapping',tapping, 'read', ncid, readvar=readv, posNOTonfile=.true.)
    if ( .not. readv ) call endrun( trim(subname)//' ERROR: error in reading in pft data' )
    call ncd_io('rain_min',rain_min, 'read', ncid, readvar=readv, posNOTonfile=.true.)
    if ( .not. readv ) call endrun( trim(subname)//' ERROR: error in reading in pft data' )
    call ncd_io('crit_dry_dur',crit_dry_dur, 'read', ncid, readvar=readv, posNOTonfile=.true.)
    if ( .not. readv ) call endrun( trim(subname)//' ERROR: error in reading in pft data' )
    call ncd_io('clearcut_yr',clearcut_yr, 'read', ncid, readvar=readv, posNOTonfile=.true.)
    if ( .not. readv ) call endrun( trim(subname)//' ERROR: error in reading in pft data' )
    call ncd_io('tap_npp',tap_npp, 'read', ncid, readvar=readv, posNOTonfile=.true.)
    if ( .not. readv ) call endrun( trim(subname)//' ERROR: error in reading in pft data' )
    call ncd_io('tap_partition',tap_partition, 'read', ncid, readvar=readv, posNOTonfile=.true.)
    if ( .not. readv ) call endrun( trim(subname)//' ERROR: error in reading in pft data' )
