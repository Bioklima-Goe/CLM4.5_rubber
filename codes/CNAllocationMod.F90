...
! !USES:
   ...
   use pftvarcon , only: npcropmin, declfact, bfact, aleaff, arootf, astemf, &
                         ...
                         nsoybean, tapping, clearcut_yr, tap_npp, tap_partition
   use clm_varcon, only: secspday, istsoil, istcrop
   use clm_varpar, only: max_pft_per_col
   use clm_time_manager , only : get_step_size, get_days_per_year, get_curr_calday, get_curr_date
   ! Ashehad Ali added tapping, clearcut_yr, tap_npp, tap_partition
   ! Ashehad Ali added get_days_per_year, get_curr_calday, get_curr_date
!
! !ARGUMENTS:
   implicit none
   integer, intent(in) :: lbp, ubp        ! pft-index bounds
...
! !OTHER LOCAL VARIABLES:
   ...
   integer kyr                     !current year  Ashehad Ali added this
   integer kmo                     !month of year  (1, ..., 12)  Ashehad Ali added this
   integer kda                     !day of month   (1, ..., 31)  Ashehad Ali added this
   integer mcsec                   !seconds of day (0, ..., seconds/day) Ashehad Ali added this
   
   real(r8) :: fcur_st_old        ! AAli (3/17/2017) (unitless) allocation parameter: fraction of storage allocation when not tapped 
   real(r8) :: fcur_tot_old       ! AAli (3/17/2017) (unitless) allocation parameter: fraction of storage and growth allocation when not tapped
   real(r8) :: fcur_tot           ! AAli (3/17/2017) (unitless) allocation parameter: fraction of storage, growth and tapped allocation 
   real(r8) :: fcur_tap           ! AAli (3/17/2017) (unitless) allocation parameter: fraction of allocation that will go to tapping
   real(r8) :: fcur_st            ! AAli (3/17/2017) (unitless) allocation parameter: fraction of allocation that will go to storage pool
   real(r8) :: fcur_gr            ! AAli (3/17/2017) (unitless) allocation parameter: fraction of allocation that will go to currently displayed growth  
   real(r8) :: fcur_tap_a         ! AAli (3/17/2017) (unitless) allocation parameter: actual fraction of allocation that will go to tapping from assimilation
   real(r8) :: fcur_gr_a          ! AAli (3/17/2017) (unitless) allocation parameter: actual fraction of allocation that will go to growth from assimilation
   real(r8) :: fcur_st_a          ! AAli (3/17/2017) (unitless) allocation parameter: actual fraction of allocation that will go to storage from assimilation  
...
!-----------------------------------------------------------------------
   ...
   cpool_to_tappingc           => pcf%cpool_to_tappingc        ! allocation to tapping C (gC/m2/s)  Ashehad Ali
   npool_to_tappingn           => pnf%npool_to_tappingn        ! allocation to tapping N (gN/m2/s)  Ashehad Ali
   ...
   do fp=1,num_soilp
      ...
      !Ashehad Ali modified below for rubber
      
      if (stem_leaf(ivt(p)) == -1._r8 .and. kyr >= clearcut_yr(ivt(p)) + 6) then
         f3 = (2.7/(1.0+exp(-0.004*((1.0_r8 - tap_npp(ivt(p))) * annsum_npp(p) - 300.0)))) - 0.4
      else
         f3 = (2.7/(1.0+exp(-0.004*(annsum_npp(p)- 300.0)))) - 0.4   
      end if
...
!               (ivt(pfti(c)) >= npcropmin)) ) then
   ...
   do fp=1,num_soilp
    ...
    ! Not necessary to repeat the allocation variables f1/f2/f3/f4/f5/g1/g2 here.
    ! Otherwise it may cancel out all previous calculations, e.g. for palm (Y.Fan)
    ...
    !  end if
      ...
      !Ashehad Ali added below for tapping 
      fcur = fcur2(ivt(p))
      fcur_st_old = 1.0_r8 - fcur                     !! AAli (10/10/2017)      
      fcur_tot_old = fcur + fcur_st_old               !  AAli (10/10/2017) 
      fcur_tot = fcur_tot_old
      
      !tapping = 1.0_r8      
      
      if (tapping(ivt(p)) > 0 .and. kyr >= clearcut_yr(ivt(p)) + 6) then
            fcur_tap = tap_partition(ivt(p))                ! <0.4  [0 0.4] is 0.2
            fcur_st =  0.5_r8                               ! >0.5  [0.5 1.0]  
            fcur_gr =  1.0_r8 - fcur_tap - fcur_st          !      
            fcur_tap_a = fcur_tot * fcur_tap                !  AAli (10/10/2017)       
            fcur_gr_a = fcur_tot * fcur_gr                  !! AAli (10/10/2017)
            fcur_st_a = fcur_tot * fcur_st                  !  AAli (10/10/2017)
         else
            fcur_tap = 0._r8
	    fcur_gr =  0.5_r8                                
            fcur_st =  0.5_r8                 
      ...
      !Ashehad Ali modifed below to include tapping
      
      cpool_to_leafc(p)          = nlc * fcur_gr_a 
      cpool_to_leafc_storage(p)  = nlc * fcur_st_a 
      cpool_to_frootc(p)         = nlc * f1 * fcur_gr_a
      cpool_to_frootc_storage(p) = nlc * f1 * fcur_st_a      
         if (tapping(ivt(p)) > 0 .and. kyr >= clearcut_yr(ivt(p)) + 6) then
            cpool_to_tappingc(p)  = nlc * fcur_tap_a + tap_npp(ivt(p)) * annsum_npp(p) * 1/(3600._r8 * 24._r8 * 365._r8)
         else
            cpool_to_tappingc(p)  = 0._r8                
         end if
      if (woody(ivt(p)) == 1._r8) then
         cpool_to_livestemc(p)          = nlc * f3 * f4 * fcur_gr_a 
         cpool_to_livestemc_storage(p)  = nlc * f3 * f4 * fcur_st_a 
         cpool_to_deadstemc(p)          = nlc * f3 * (1._r8 - f4) * fcur_gr_a 
         cpool_to_deadstemc_storage(p)  = nlc * f3 * (1._r8 - f4) * fcur_st_a 
         cpool_to_livecrootc(p)         = nlc * f2 * f3 * f4 * fcur_gr_a 
         cpool_to_livecrootc_storage(p) = nlc * f2 * f3 * f4 * fcur_st_a 
         cpool_to_deadcrootc(p)         = nlc * f2 * f3 * (1._r8 - f4) * fcur_gr_a
         cpool_to_deadcrootc_storage(p) = nlc * f2 * f3 * (1._r8 - f4) * fcur_st_a
      end if
      if (ivt(p) >= npcropmin) then ! skip 2 generic crops
      ...
      npool_to_leafn(p)          = (nlc / cnl) * fcur_gr_a 
      npool_to_leafn_storage(p)  = (nlc / cnl) * fcur_st_a 
      npool_to_frootn(p)         = (nlc * f1 / cnfr) * fcur_gr_a
      npool_to_frootn_storage(p) = (nlc * f1 / cnfr) * fcur_st_a 
         if (tapping(ivt(p)) > 0 .and. kyr >= clearcut_yr(ivt(p)) + 6) then
           npool_to_tappingn(p)       = (nlc / cnl) * fcur_tap_a + 1/cnl * tap_npp(ivt(p)) * annsum_npp(p) * 1/(3600._r8 * 24._r8 * 365._r8)  
         else
            npool_to_tappingn(p)  = 0._r8                
         end if                                  
      if (woody(ivt(p)) == 1._r8) then
         npool_to_livestemn(p)          = (nlc * f3 * f4 / cnlw) * fcur_gr_a 
         npool_to_livestemn_storage(p)  = (nlc * f3 * f4 / cnlw) * fcur_st_a 
         npool_to_deadstemn(p)          = (nlc * f3 * (1._r8 - f4) / cndw) * fcur_gr_a 
         npool_to_deadstemn_storage(p)  = (nlc * f3 * (1._r8 - f4) / cndw) * fcur_st_a 
         npool_to_livecrootn(p)         = (nlc * f2 * f3 * f4 / cnlw) * fcur_gr_a
         npool_to_livecrootn_storage(p) = (nlc * f2 * f3 * f4 / cnlw) * fcur_st_a 
         npool_to_deadcrootn(p)         = (nlc * f2 * f3 * (1._r8 - f4) / cndw) * fcur_gr_a
         npool_to_deadcrootn_storage(p) = (nlc * f2 * f3 * (1._r8 - f4) / cndw) * fcur_st_a
      end if
      if (ivt(p) >= npcropmin) then ! skip 2 generic crops
